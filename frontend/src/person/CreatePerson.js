    ...
    fetch(`http://${window.location.hostname}:9080people`, {
      ...
      .then(response => {
        ...
        response.knows?.forEach(element => {
          i++;
          let temp = { Code: element.code, Primary: element.primary, Experience: element.experience, Level: element.level }
        ...
        response.certificates?.forEach(element => {
          i++;
          let temp = { Code: element.code, Name: element.name, Comments: element.comments, Date: element.date }
