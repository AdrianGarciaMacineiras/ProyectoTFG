...
import com.tfg.skilltree.fixtures.PersonFixtures;
...
class PeopleControllerTest {
  ...
  void getPersonByCode() throws Exception {
    when(peopleService.findByCode(anyString())).thenReturn(PersonFixtures.PEOPLE_BY_CODE);
    mockMvc.perform(MockMvcRequestBuilders
                    .get("/api/person/1")
                    .accept(MediaType.APPLICATION_JSON))
            .andExpect(content().json(PersonFixtures.PERSON_BY_CODE_JSON));
  }
  ...
  void updatePerson() throws Exception {
    when(peopleUpdater.update(anyString(), any(People.class))).thenReturn(PersonFixtures.UPDATED_PEOPLE_BY_CODE);
    mockMvc.perform(MockMvcRequestBuilders
                    .put("/api/person/1")
                    .contentType(MediaType.APPLICATION_JSON)
                    .accept(MediaType.APPLICATION_JSON)
                    .content(PersonFixtures.PERSON_BY_CODE_JSON))
            .andExpect(status().isOk())
            .andExpect(content().json(PersonFixtures.UPDATED_PERSON_BY_CODE_JSON));
  }

  @Test
  void addPerson() throws Exception {
    when(peopleService.create(any(People.class))).thenReturn(PersonFixtures.PEOPLE_BY_CODE);
    mockMvc.perform(MockMvcRequestBuilders
                    .post("/api/people")
                            .contentType(MediaType.APPLICATION_JSON)
                            .accept(MediaType.APPLICATION_JSON)
  ...
  void patchPerson() throws Exception {
    when(peopleUpdater.patch(anyString(), any(People.class))).thenReturn(PersonFixtures.UPDATED_PEOPLE_BY_CODE);
    mockMvc.perform(MockMvcRequestBuilders
                    .patch("/api/person/1")
                      .contentType(MediaType.APPLICATION_JSON)
                      .accept(MediaType.APPLICATION_JSON)
                      .content(PersonFixtures.PATCH_PERSON_BY_CODE_JSON))
           .andDo(log())
           .andExpect(content().json(PersonFixtures.PATCH_PERSON_BY_CODE_JSON));
  }

  @Test
  void getPeople() throws Exception {
    when(peopleService.getAll()).thenReturn(PersonFixtures.PEOPLE_LIST);
    mockMvc.perform(MockMvcRequestBuilders
                    .get("/api/people")
                            .accept(MediaType.APPLICATION_JSON))
            .andExpect(content().json(PersonFixtures.LIST_PERSON_JSON));
  }
